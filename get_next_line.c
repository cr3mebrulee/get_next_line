...
#include "get_next_line.h"
	...
	//read_line() read file till the \n character and concatenate it with the string 
	//stored in static variable written in previous call
	//why we dont free buf: read system call does not append data to the buffer; 
	//instead, it overwrites the existing content of the buffer with the newly read data.

static char	*read_line(int fd, char *buf, char *backup)
{
    	//declare variable i to store the result of read()
    	//and additional pointer
	ssize_t		i;
	char		*char_temp;	
	i = 1;
	...
	if (i == -1)
        ...
        //null terminating buf like a string for further 
	//string manipulation operation
	buf[i] = '\0';
	
        //in first call of the read_line() backup points to NULL
        //we write there "" that ft_strjoin will take it like a string
	if (backup == NULL)
	{
		backup = ft_strdup("");
	}
       
       //assigning backup to char_temp for safe cancatenation strings in to backup
	char_temp = backup;
       
        //cncatenate the data from previos read data with current data in buf
	backup = ft_strjoin(char_temp, buf);
       
        //the original content of backup is freed to avoid memory leaks.
	free(char_temp);
	char_temp = NULL;
...
static char	*extract(char *line)
{
	...
	count = 0;
    	...
    	//let's us find the end of the line either when 
	//we encounter a \n or a \0
	while (line[count] != '\n' && line[count] != '\0')
		count++;
	...
	if (line[count] == '\0' || line[1] == '\0')
    	...
    	//create a substring starting from the character 
	//after the \n character (count + 1) till the end of string
...
char	*get_next_line(int fd)
{
	...
	static char	*backup;
    	...
    	//and if there are mistakes during the read operation 
	//(forexample if file has no read permissions)
	...
	if (!buf)
    	...
    	//call function to read new line and concatenate it with 
    	//the data stored in backup from previous calls of get_next_line()
